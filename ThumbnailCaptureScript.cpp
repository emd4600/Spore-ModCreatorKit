...
void ThumbnailCaptureScript::CaptureImage() {
	auto device = RenderWare::Renderer::GetDevice();
	HRESULT hr;
...
void ThumbnailCaptureScript::InjectListeners() {
	...
	Graphics::RenderManager()->AddRenderable(this, 40);
}
...
void ThumbnailCaptureScript::RemoveListeners() {
	...
	Graphics::RenderManager()->RemoveRenderable(40);
}
...
void ThumbnailCaptureScript::ParseLine(const ArgScript::Line& line) {
	...
	else if (!GetEditor()->IsMode(Editor::Mode::BuildMode)) {
...
void ThumbnailCaptureScript::AddCheat() {
	intrusive_ptr<ThumbnailCaptureScript> script = new ThumbnailCaptureScript();

	App::CheatManager()->AddCheat("captureIcon", script.get());
}
